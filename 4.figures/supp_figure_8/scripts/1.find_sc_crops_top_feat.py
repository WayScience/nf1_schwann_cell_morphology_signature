...
# Find the top negative feature/third most important feature (predicting Null) [NOT INCLUDED AS MONTAGE]
top_Null_idx = feat_import_df["feature_importances"].idxmin()
top_Null_feature = feat_import_df.loc[top_Null_idx, "feature_names"]
top_Null_value = feat_import_df.loc[top_Null_idx, "feature_importances"]

# Print the feature name and its value
print(f"{top_Null_feature}: {top_Null_value}")
...
# ### Max single-cells for top Null feature (Null are max)

# In[8]:


# Get data frame with the next top 6 single-cells
max_Null_feature = filtered_plate5_df[
    filtered_plate5_df["Metadata_genotype"] == "Null"
].nlargest(6, top_Null_feature)[
    [
        top_Null_feature,
        "Metadata_genotype",
        "Metadata_Well",
...
# ### Min single-cells for top Null feature (WT are min)

# In[9]:


# Get data frame with the top 6 single-cells
min_Null_feature = filtered_plate5_df[
    filtered_plate5_df["Metadata_genotype"] == "WT"
].nsmallest(6, top_Null_feature)[
    [
        top_Null_feature,
        "Metadata_genotype",
        "Metadata_Well",
